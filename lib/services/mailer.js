...
module.exports = class MailerService extends Service {
    ...
    async filmUpdateNotification(id,titre) {

        const transporter = this.createTransporter();

        const { User } = this.server.models();
        const users = await User.query().select('email').join('favouriteFilms', 'user.id', '=', 'favouritefilms.idUser').where('favouritefilms.idFilm',id);
        const emails = this.concatUserEmail(users);
        ...
        await transporter.sendMail({
            ...
            text: 'Un film vien d\'être modifé : ' + titre
        });
    }
